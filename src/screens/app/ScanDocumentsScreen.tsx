...
// ---- helpers (logic only, no UI changes) ----
...
const ScanDocumentScreen = ({ navigation }: ScanLetterScreenProps) => {
  const [scannedDocuments, setScannedDocuments] = useState<Array<ScannedDoc>>(
    []
  );
  ...
  const handleImportFromFiles = async () => {
    try {
      ...
      const imported: ScannedDoc[] = result.assets.map((asset) => {
        ...
        const finalMime =
          asset.mimeType || guessMimeTypeFromName(asset.name || "file");
        try {
          src.copy(dest);
          return {
  ...
  const handlePickFromPhotos = async () => {
    ...
    const permStatus = await ImagePicker.getMediaLibraryPermissionsAsync(true);
    if (!permStatus.granted) {
      const req = await ImagePicker.requestMediaLibraryPermissionsAsync(true);
      if (!req.granted) {
        Alert.alert(
          "Permission required",
          "Allow photo access to import from your gallery."
        );
        return;
      }
    }
    ...
    try {
      ...
      const importedFromLibrary: ScannedDoc[] = res.assets.map((asset) => {
        // expo-image-picker asset fields:
        // asset.uri, asset.fileName, asset.mimeType
        const src = new File(asset.uri);
        const safeName =
          asset.fileName ||
          `photo_${Date.now()}_${Math.floor(Math.random() * 1000)}.jpg`;
        const dest = new File(lettersDir, safeName);

        const finalMime =
          asset.mimeType || guessMimeTypeFromName(safeName || "image.jpg");
      ...
      setScannedDocuments((prev) => [...importedFromLibrary, ...prev]);
    } catch (e) {
      console.warn("pickFromPhotos error", e);
      Alert.alert(
        "Import failed",
        "Could not import from photos. Please try again."
      );
    }
  };
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      setProgress((prev) => Math.min(prev + STEP_PER_TICK, 100));
    }, TICK_MS);
    ...
    >
      ...
      {scannedDocuments?.length > 0 && (
        <Button
          ...
          onPress={() => {
            // Build the shape expected by the processor hook
            const uploads = scannedDocuments.map((d) => ({
              uri: d.filePath,
              name: d.fileName,
              mimeType: d.mimeType,
            }));
            navigation.navigate(SUMMARY, { uploads });
          }}
          loading={false}
          disabled={false}
